class Boggle:
    ...
    def _dfs(self, r, c, current_word, visited, found_words):
        ...
        if (current_word not in self.prefixes and
                current_word not in self.dictionary):
            visited[r][c] = False
            return

        # Only add valid words (>=3 chars), not ending with forbidden letters
        if len(current_word) >= 3 and current_word in self.dictionary:
            if not (current_word.endswith("S") or
                    current_word.endswith("I") or
                    current_word.endswith("ST")):
